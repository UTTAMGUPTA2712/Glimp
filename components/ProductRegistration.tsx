...
export default function ProductRegistration() {
  ...
  const checkRegistration = async () => {
    try {
      const res = await fetch(`/api/product/register?userId=${userId}`);
      const data = await res.json();
      
      if (data.registered) {
        setIsRegistered(true);
        setRegisteredUuid(data.data.uuid);
        setMessage({ 
          type: 'success', 
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/product/register', {
        ...
        body: JSON.stringify({ uuid: uuid.trim(), userId })
      });
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
        ...
        {isRegistered ? (
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full text-blue-600 py-2 px-4 rounded-lg font-medium hover:bg-blue-50 transition-colors text-sm"
            >
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          <p className="text-xs text-gray-500 text-center">
            Your product can only be used on one device at a time. 
            Registration prevents unauthorized sharing.
          </p>
        </div>
      </div>
    </>
